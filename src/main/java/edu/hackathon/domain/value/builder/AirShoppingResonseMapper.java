...
import edu.hackathon.util.AncillaryType;
...
public class AirShoppingResonseMapper {
	...
	public Offers mapAirShoppingResponse(AirShoppingRS response) {
		...
		Map<String, List<Object>> flightAncilariesLookup = getFlightAncilariesLookup(offersGroup.getAirlineOffers()
				.get(0).getAirlineOffer());
		...
		RecommendedItinerary recomendedItinerary = identifyRecommendedItinerary(flightList.get(0), segmentList,
				flightAncilariesLookup, ancilaryServicesLookupMap);
	...
	private Map<String, List<Object>> getFlightAncilariesLookup(List<AirlineOffer> airlineOffers) {
		...
		for (AirlineOffer airlineOffer : airlineOffers) {
			flightAncilariesLookup.put(airlineOffer.getPricedOffer().getAssociations().get(0).getApplicableFlight()
					.getFlightReferences().toString(), airlineOffer.getPricedOffer().getAssociations().get(0)
					.getAssociatedService().getServiceReferences());
		}
		return flightAncilariesLookup;
	}

	private RecommendedItinerary identifyRecommendedItinerary(Flight singleFlight,
			List<ListOfFlightSegmentType> flightSegmentList, Map<String, List<Object>> flightAncilariesLookup,
			Map<String, String> ancilaryServicesLookupMap) {
		List<AvailableAncilaries> availableAncilaries = null;
		...
		if (includedAncilaries != null) {
			availableAncilaries = new ArrayList<AvailableAncilaries>();
			for (Object object : includedAncilaries) {
				...
				ancilaries.setAncillaryType(AncillaryType.Meal.name());
				ancilaries.setAncillaryUrl(null);
